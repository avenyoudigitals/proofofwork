...
// Deterministic gradient per work based on title
const GRADIENTS = [
...
function Avatar({ name, avatarUrl, size = 80 }: { name: string; avatarUrl: string | null; size?: number }) {
  ...
  if (avatarUrl) {
    return (
      ...
      <img src={avatarUrl} alt={name} style={{ width: size, height: size, borderRadius: '50%', objectFit: 'cover', border: '3px solid #e5e5e8' }} />
    )
  }
  return (
    <div style={{
      ...
      border: '3px solid #e5e5e8', flexShrink: 0,
    }}>
      {initials}
    </div>
  )
  ...
  return (
    <div style={{
      ...
      <div style={{ height: 140, background: gradient, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {/* Time */}
        <span style={{ position: 'absolute', bottom: 10, left: 14, fontSize: 10, color: 'rgba(255,255,255,0.65)', fontFamily: 'var(--font-mono)' }}>
      ...
      <div style={{ padding: '16px 18px' }}>
        <p style={{ fontSize: 14, fontWeight: 700, color: '#0a0a12', letterSpacing: '-0.02em', marginBottom: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {work.title}
        </p>
        <p style={{ fontSize: 12, color: '#6b7280', marginBottom: work.description ? 10 : 14 }}>
          {work.role}{work.company ? ` · ${work.company}` : ''}
        </p>
        {work.description && (
          <p style={{ fontSize: 12, color: '#55556e', lineHeight: 1.6, marginBottom: 14, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
            {work.description}
          </p>
        )}
        ...
        {work.tags && work.tags.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: links.length ? 14 : 0 }}>
            {work.tags.slice(0, 4).map(t => (
              <span key={t} style={{ fontSize: 10, padding: '2px 8px', border: '1px solid #e5e5e8', color: '#6b7280', borderRadius: 99, fontFamily: 'var(--font-mono)' }}>{t}</span>
            ))}
          </div>
        )}
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {links.map(l => (
              <a key={l.label} href={l.href} target="_blank" rel="noopener noreferrer"
                style={{ fontSize: 10, fontWeight: 600, color: '#6366f1', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 3, padding: '3px 9px', border: '1px solid rgba(99,102,241,0.25)', borderRadius: 99, background: 'rgba(99,102,241,0.06)' }}>
                {l.icon} {l.label}
              </a>
            ))}
          </div>
    ...
    <div style={{
      ...
      <div style={{ flex: 1, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16, minWidth: 0 }}>
        ...
        <div style={{ width: 44, height: 44, borderRadius: 12, background: gradient, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <span style={{ fontSize: 16, fontWeight: 800, color: '#fff', fontFamily: 'var(--font-mono)' }}>
            {work.title.slice(0, 2).toUpperCase()}
          </span>
        </div>
        ...
        <div style={{ display: 'flex', gap: 5 }}>
          {(work.tags ?? []).slice(0, 3).map(t => (
            <span key={t} style={{ fontSize: 10, padding: '2px 8px', border: '1px solid #e5e5e8', color: '#6b7280', borderRadius: 99, fontFamily: 'var(--font-mono)', whiteSpace: 'nowrap' }}>{t}</span>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 5, flexShrink: 0 }}>
          {links.map(l => (
            <a key={l.label} href={l.href} target="_blank" rel="noopener noreferrer"
              style={{ fontSize: 11, color: '#6366f1', textDecoration: 'none', padding: '3px 9px', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 99, background: 'rgba(99,102,241,0.06)', whiteSpace: 'nowrap' }}>
              {l.icon} {l.label}
            </a>
          ))}
        </div>
...
export default function PublicProfileClient({
  userId, displayName, username, bio, avatarUrl,
  ...
  return (
    <>
      <style>{`
        ...
        .view-btn:hover { background: #f0f0f8 !important; }
        .view-btn.active { background: #0a0a12 !important; color: #fff !important; }
      ...
      <div style={{ minHeight: '100vh', background: '#f8f8fc', fontFamily: 'var(--font)' }}>
        ...
        <div style={{ background: '#fff', borderBottom: '1px solid #e8e8f0', position: 'sticky', top: 0, zIndex: 20 }}>
          <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 24px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Link href="/" style={{ textDecoration: 'none', fontSize: 15, fontWeight: 800, color: '#0a0a12', letterSpacing: '-0.03em' }}>
              ProofForge
            </Link>
            <div style={{ display: 'flex', gap: 8 }}>
              <Link href="/login" style={{ fontSize: 13, fontWeight: 500, color: '#55556e', textDecoration: 'none', padding: '7px 16px', borderRadius: 99, border: '1.5px solid #e5e5e8' }}>
                Sign in
              </Link>
              <Link href="/signup" style={{ fontSize: 13, fontWeight: 600, color: '#fff', textDecoration: 'none', padding: '7px 18px', borderRadius: 99, background: '#0a0a12' }}>
                Get started
              </Link>
            </div>
          </div>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 24, flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: 1, minWidth: 220 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 6 }}>
                  <h1 style={{ fontSize: 28, fontWeight: 800, color: '#0a0a12', letterSpacing: '-0.04em', lineHeight: 1 }}>
                    {displayName}
                  </h1>
                ...
                {username && (
                  <p style={{ fontSize: 13, color: '#9ca3af', marginBottom: 10, fontFamily: 'var(--font-mono)' }}>@{username}</p>
                )}

                {bio && (
                  <p style={{ fontSize: 14, color: '#55556e', lineHeight: 1.7, maxWidth: 480, marginBottom: 14 }}>{bio}</p>
                )}
                ...
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  {website && (
                    <a href={website} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: '#6366f1', textDecoration: 'none' }}>
                      {website.replace(/^https?:\/\//, '')}
                    </a>
                  )}
                  {githubUsername && (
                    <a href={`https://github.com/${githubUsername}`} target="_blank" rel="noopener noreferrer"
                      style={{ fontSize: 12, color: '#55556e', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 12 }}>
                ...
                >
                  {copied ? (
                    ...
                    <>
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71" strokeLinecap="round" strokeLinejoin="round" /><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71" strokeLinecap="round" strokeLinejoin="round" /></svg>
                      Share profile
                    </>
                  )}
                </button>
            ...
            <div style={{ display: 'flex', background: '#f0f0f8', borderRadius: 12, padding: 4, gap: 2 }}>
              <button
                ...
                style={{
                  ...
                  background: view === 'grid' ? '#0a0a12' : 'transparent',
              ...
              <button
                ...
                style={{
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  width: 36, height: 36, borderRadius: 9, border: 'none',
                  background: view === 'list' ? '#0a0a12' : 'transparent',
        ...
        <div style={{ borderTop: '1px solid #e8e8f0', background: '#fff' }}>
          <div style={{ maxWidth: 960, margin: '0 auto', padding: '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
            <Link href="/" style={{ fontSize: 13, fontWeight: 800, color: '#0a0a12', letterSpacing: '-0.03em', textDecoration: 'none' }}>
              ProofForge
            </Link>
